...
# Функція для створення нотаток, у випадку коли нотатка створена, запитуємо чи бажає користувач створити ще одну. Якщо відповідь ні то повертаємось до головного меню.
def add_note():
    while True:
...
# Функція для сортування нотаток з урахуванням помилки у разі якщо нотаток немає.
def sort_notes(order, num_notes):
    if not notes:
        return 'Неможливо сортувати, оскільки Ви не додали жодної нотатки.'
